...
import { DeleteChartDocument, GetChartDocumentBatch } from '~/utils/request'
...
function submitForm() {
  vFormRef.value.getFormData().then((formData: any) => {
    router.push(`/edit/document/${formData.path}/${formData.name}`)
  }).catch((error: any) => {
...
async function handleDelete(index: number) {
  const { data } = await DeleteChartDocument(tableData.value[index]).send(true)
    ...
    <ElTable :data="tableData" table-layout="auto" flexible>
      ...
      <el-table-column prop="path" label="Path" width="150">
        <template #default="scope">
          <RouterLink :to="`/view/document/${scope.row.path}`">
            {{ scope.row.path }}
          </RouterLink>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="document" label="document" width="120">
        <template #default="scope">
          <el-text truncated w-15>
            {{ scope.row.document }}
          </el-text>
        </template>
      </el-table-column>
